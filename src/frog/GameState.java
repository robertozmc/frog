...
/**
 * Klasa - stan, będąca pierwszym poziomem gry 
...
public class GameState extends BasicGameState {
	...
	public void update(GameContainer gc, StateBasedGame sbg, int t) throws SlickException {
		LoadData.level1Time -= t; 
		LoadData.timePassed += t;
		if (LoadData.timePassed > 3000 + LoadData.random.nextInt(2000)) {
			LoadData.timePassed = 0;
			LoadData.flies.add(new Circle(gc.getWidth()+LoadData.circleRadius,
					LoadData.circleRadius+LoadData.random.nextInt(gc.getHeight()/3), LoadData.circleRadius));
		}
		
		for (Circle c : LoadData.flies) {
			c.setCenterX(c.getCenterX()-(t/8));
		}
		
		for (int i = LoadData.flies.size()-1; i >= 0; i--) {
			Circle c = LoadData.flies.get(i);
			if (c.getCenterX() < -50) {
				LoadData.flies.remove(i);
		...
		} else if (LoadData.level1Time/1000 <= 0) {
			LoadData.totalPoints = LoadData.points1+LoadData.points2+LoadData.points3;
	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException {
		...
		LoadData.uniFont.drawString(10, gc.getHeight()-100, "Czas: " + LoadData.level1Time/1000);
		LoadData.uniFont.drawString(10+LoadData.uniFont.getWidth("Czas: " + LoadData.level1Time/1000)*2, gc.getHeight()-100, "Punkty: " + LoadData.points1);
		...
		for (Circle c : LoadData.flies) {
			g.fill(c);
			LoadData.fly.draw(c.getCenterX()-50, c.getCenterY()-50);
		}
		if (gc.getInput().isMouseButtonDown(Input.MOUSE_LEFT_BUTTON)) {
